...
contract Permit2BundlerForkTest is ForkTest {
    ...
    function testApprove2Batch(uint256 privateKey, uint256 amount0, uint256 amount1) public {
        ...
        (uint160 permit2Allowance1,,) = Permit2Lib.PERMIT2.allowance(user, token0, address(genericBundler1));

        (uint160 permit2Allowance2,,) = Permit2Lib.PERMIT2.allowance(user, token1, address(genericBundler1));

        assertEq(permit2Allowance1, amount0, "PERMIT2.allowance(user, asset 1, genericBundler1)");
        assertEq(permit2Allowance2, amount1, "PERMIT2.allowance(user, asset 2,genericBundler1)");
        assertEq(
            ERC20(token0).allowance(user, address(genericBundler1)), 0, "loan.allowance(user, asset 1, genericBundler1)"
        );
        assertEq(
            ERC20(token1).allowance(user, address(genericBundler1)), 0, "loan.allowance(user, asset 2, genericBundler1)"
        );
    }
    ...
    function testApprove2BatchUnauthorized() public {
        ...
        genericBundler1.approve2Batch(permitBatch, signature, false);
    }
