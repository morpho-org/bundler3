...
contract AaveV2MigrationBundlerForkTest is MigrationForkTest {
    ...
    AaveV2MigrationBundlerV2 public migrationBundler;
    ...
    function setUp() public override {
        ...
        migrationBundler = new AaveV2MigrationBundlerV2(address(hub), AAVE_V2_POOL);
    }
    ...
    function testAaveV2RepayUnauthorized(uint256 amount) public onlyEthereum {
        ...
        migrationBundler.aaveV2Repay(marketParams.loanToken, amount, 1);
    }
    ...
    function testAaveV2WithdrawUnauthorized(uint256 amount) public onlyEthereum {
        ...
        vm.expectPartialRevert(ErrorsLib.UnauthorizedSender.selector);
        migrationBundler.aaveV2Withdraw(marketParams.loanToken, amount, address(this));
    }
    ...
    function testMigrateBorrowerWithPermit2(uint256 privateKey) public onlyEthereum {
        ...
        callbackBundle.push(_aaveV2Withdraw(marketParams.collateralToken, collateralSupplied, address(genericBundler1)));
    ...
    function testMigrateBorrowerDaiToSDaiWithPermit2(uint256 privateKey) public onlyEthereum {
        ...
        callbackBundle.push(_approve2(privateKey, aToken, uint160(aTokenBalance), 0, false));
        callbackBundle.push(_transferFrom2(aToken, address(migrationBundler), aTokenBalance));
        callbackBundle.push(_aaveV2Withdraw(DAI, collateralSupplied, address(genericBundler1)));
        callbackBundle.push(_erc4626Deposit(S_DAI, collateralSupplied, 0, address(genericBundler1)));
    ...
    function testMigrateStEthPositionWithPermit2(uint256 privateKey) public onlyEthereum {
        ...
        callbackBundle.push(_transferFrom2(aToken, address(migrationBundler), aTokenBalance));
        callbackBundle.push(_aaveV2Withdraw(ST_ETH, type(uint256).max, address(ethereumBundler1)));
        callbackBundle.push(_wrapStEth(type(uint256).max, address(genericBundler1)));
    ...
    function testMigrateSupplierWithPermit2(uint256 privateKey, uint256 supplied) public onlyEthereum {
        ...
        bundle.push(_aaveV2Withdraw(marketParams.loanToken, supplied, address(genericBundler1)));
    ...
    function _aaveV2Repay(address token, uint256 amount) internal view returns (Call memory) {
        return _call(migrationBundler, abi.encodeCall(migrationBundler.aaveV2Repay, (token, amount, RATE_MODE)));
    }

    function _aaveV2Withdraw(address token, uint256 amount, address receiver) internal view returns (Call memory) {
        return _call(migrationBundler, abi.encodeCall(migrationBundler.aaveV2Withdraw, (token, amount, receiver)));
    }
}
