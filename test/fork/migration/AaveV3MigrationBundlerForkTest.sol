...
contract AaveV3MigrationBundlerForkTest is MigrationForkTest {
    ...
    AaveV3MigrationBundlerV2 public migrationBundler;
    ...
    function setUp() public override {
        ...
        migrationBundler = new AaveV3MigrationBundlerV2(address(hub), address(AAVE_V3_POOL));
        vm.label(address(migrationBundler), "Aave V3 Migration Bundler");
    }
    ...
    function testAaveV3RepayUnauthorized(uint256 amount) public {
        ...
        migrationBundler.aaveV3Repay(marketParams.loanToken, amount, 1);
    }
    ...
    function testAaveV3WithdrawUnauthorized(uint256 amount) public {
        ...
        vm.expectPartialRevert(ErrorsLib.UnauthorizedSender.selector);
        migrationBundler.aaveV3Withdraw(marketParams.loanToken, amount, address(this));
    }
    ...
    function testMigrateBorrowerWithATokenPermit(uint256 privateKey) public {
        ...
        callbackBundle.push(_aaveV3PermitAToken(aToken, privateKey, address(genericBundler1), aTokenBalance));
        callbackBundle.push(_erc20TransferFrom(aToken, address(migrationBundler), aTokenBalance));
        callbackBundle.push(_aaveV3Withdraw(marketParams.collateralToken, collateralSupplied, address(genericBundler1)));
    ...
    function testMigrateBorrowerWithPermit2(uint256 privateKey) public {
        ...
        callbackBundle.push(_transferFrom2(aToken, address(migrationBundler), aTokenBalance));
        callbackBundle.push(_aaveV3Withdraw(marketParams.collateralToken, collateralSupplied, address(genericBundler1)));
    ...
    function testMigrateUSDTPositionWithPermit2(uint256 privateKey) public onlyEthereum {
        ...
        callbackBundle.push(_approve2(privateKey, aToken, uint160(aTokenBalance), 0, false));
        callbackBundle.push(_transferFrom2(aToken, address(migrationBundler), aTokenBalance));
        callbackBundle.push(_aaveV3Withdraw(USDT, amountUsdt, address(genericBundler1)));
    ...
    function testMigrateSupplierWithATokenPermit(uint256 privateKey, uint256 supplied) public {
        ...
        bundle.push(_aaveV3PermitAToken(aToken, privateKey, address(genericBundler1), aTokenBalance));
        bundle.push(_erc20TransferFrom(aToken, address(migrationBundler), aTokenBalance));
        bundle.push(_aaveV3Withdraw(marketParams.loanToken, supplied, address(genericBundler1)));
    ...
    function testMigrateSupplierWithPermit2(uint256 privateKey, uint256 supplied) public {
        ...
        bundle.push(_transferFrom2(aToken, address(migrationBundler), aTokenBalance));
        bundle.push(_aaveV3Withdraw(marketParams.loanToken, supplied, address(genericBundler1)));
        bundle.push(_morphoSupply(marketParams, supplied, 0, 0, user));
    ...
    function _aaveV3PermitAToken(address aToken, uint256 privateKey, address bundler, uint256 amount)
        internal
        view
        returns (Call memory)
    {
        ...
        Permit memory permit = Permit(user, bundler, amount, nonce, SIGNATURE_DEADLINE);
    ...
    function _aaveV3Repay(address asset, uint256 amount) internal view returns (Call memory) {
        return _call(migrationBundler, abi.encodeCall(AaveV3MigrationBundlerV2.aaveV3Repay, (asset, amount, RATE_MODE)));
    }

    function _aaveV3Withdraw(address asset, uint256 amount, address receiver) internal view returns (Call memory) {
        return
            _call(migrationBundler, abi.encodeCall(AaveV3MigrationBundlerV2.aaveV3Withdraw, (asset, amount, receiver)));
    }
}
