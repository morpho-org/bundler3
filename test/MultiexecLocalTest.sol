...
contract MultiexecLocalTest is LocalTest {
    ...
    function setUp() public override {
        super.setUp();
        adapterMock = new AdapterMock(address(multiexec));
        empty = address(new Empty());
    }

    function testMultiexecZeroAddress() public {
    ...
    function testAlreadyInitiated(address initiator) public {
        ...
        bundle.push(_call(adapterMock, abi.encodeCall(AdapterMock.callbackMultiexecWithMulticall, ())));
    ...
    function testInitiatorReset(address initiator) public {
        ...
        assertEq(multiexec.initiator(), address(0));
    ...
    function testPassthroughValue(address initiator, uint128 value) public {
        ...
        vm.prank(initiator);
        multiexec.multicall{value: value}(bundle);
    }

    function testNestedCallbackAndlastUnreturnedCalleeValue(address initiator) public {
        vm.assume(initiator != address(0));
        AdapterMock adapterMock2 = new AdapterMock(address(multiexec));
        AdapterMock adapterMock3 = new AdapterMock(address(multiexec));

        callbackBundle2.push(_call(adapterMock2, abi.encodeCall(AdapterMock.isProtected, ())));

        callbackBundle.push(_call(adapterMock2, abi.encodeCall(AdapterMock.callbackMultiexec, (callbackBundle2))));

        callbackBundle.push(_call(adapterMock3, abi.encodeCall(AdapterMock.callbackMultiexec, (callbackBundle2))));

        bundle.push(_call(adapterMock, abi.encodeCall(AdapterMock.callbackMultiexec, (callbackBundle))));
    ...
    function testProtectedFailure(address initiator, address adapter, address caller) public {
        ...
        _delegatePrank(address(multiexec), abi.encodeCall(FunctionMocker.setlastUnreturnedCallee, (adapter)));
        _delegatePrank(address(multiexec), abi.encodeCall(FunctionMocker.setInitiator, (initiator)));

        vm.expectRevert(ErrorsLib.UnauthorizedSender.selector);
        vm.prank(caller);
        multiexec.reenter(new Call[](0));
    }
