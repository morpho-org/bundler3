...
import {Multiexec, Call} from "../../src/Multiexec.sol";
...
abstract contract CommonTest is Test {
    ...
    function setUp() public virtual {
        ...
        generalAdapter1 = new GeneralAdapter1(address(multiexec), address(morpho), address(new WethContract()));
        paraswapAdapter = new ParaswapAdapter(address(multiexec), address(morpho), address(augustusRegistryMock));
    ...
    function _morphoFlashLoan(address token, uint256 amount, bytes memory data) internal view returns (Call memory) {
        return _call(generalAdapter1, abi.encodeCall(GeneralAdapter1.morphoFlashLoan, (token, amount, data)));
    }
