...
contract ParaswapModuleForkTest is ForkTest {
    ...
    uint256 sellAmount = 5000000;
    uint256 sellAmountOffset = 68;
    uint256 sellExpectedBuyAmount = 5e18;
    ...
    function testSellNoAdjustment(uint256 privateKey, uint256 extra) public onlyEthereum {
        ...
        deal(USDC, user, sellAmount + extra);
        ...
        assertEq(ERC20(DAI).balanceOf(address(user)), sellExpectedBuyAmount, "bought");
    }
    ...
    function testSellWithAdjustment(uint256 privateKey, uint256 percent) public onlyEthereum {
        ...
        deal(USDC, user, sellAmount * percent / 100);
        ...
        bundle.push(
            _moduleCall(
                address(paraswapModule),
                _paraswapSell(
                    AUGUSTUS_V6_2, sellCalldata, USDC, DAI, sellExpectedBuyAmount, true, sellAmountOffset, address(user)
                )
            )
        );
        ...
        assertEq(ERC20(DAI).balanceOf(address(user)), sellExpectedBuyAmount * percent / 100, "bought");
    }
    ...
    uint256 buyAmount = 1e8;
    uint256 buyAmountOffset = 132;
    uint256 expectedSellAmount = 65_549_161_928;
    ...
    function testBuyNoAdjustment(uint256 privateKey) public onlyEthereum {
        ...
        assertEq(sold, expectedSellAmount, "sold");
        assertEq(ERC20(WBTC).balanceOf(address(user)), buyAmount, "bought");
    }
    ...
    function testBuyWithAdjustment(uint256 privateKey, uint256 percent) public onlyEthereum {
        ...
        // Cannot go above 100% because Augustus will often pull the maxSellAmount encoded in the calldata upfront.
        ...
        uint256 debt = buyAmount * percent / 100;
        uint256 initialBalance = 1_000_000_000e6;
        ...
        vm.prank(user);
        ERC20(WBTC).approve(address(morpho), type(uint256).max);
        _supplyCollateral(wethWbtcMarketParams, debt * 2, address(user));
        _supply(wethWbtcMarketParams, debt * 2, address(user));
        _borrow(wethWbtcMarketParams, debt, address(user));
        vm.prank(user);
        ERC20(WBTC).transfer(address(1), debt);
        ...
        bundle.push(
            _moduleCall(
                ...
                _paraswapBuy(
                    ...
                    USDC,
                    WBTC,
                    expectedSellAmount * 110 / 100,
                    wethWbtcMarketParams,
                    buyAmountOffset,
                    address(user)
                )
            )
        ...
        uint256 sold = initialBalance - ERC20(USDC).balanceOf(address(user));
        assertApproxEqRel(sold, expectedSellAmount * percent / 100, 10 * WAD / 100, "sold");
