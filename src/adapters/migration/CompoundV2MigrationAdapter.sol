...
contract CompoundV2MigrationAdapter is CoreAdapter {
    ...
    constructor(address multiexec, address cEth) CoreAdapter(multiexec) {
    ...
    function compoundV2RepayErc20(address cToken, uint256 amount, address onBehalf) external onlyMultiexec {
    ...
    /// debt).
    /// @param onBehalf The account on behalf of which the debt is repaid.
    function compoundV2RepayEth(uint256 amount, address onBehalf) external onlyMultiexec {
    ...
    function compoundV2RedeemErc20(address cToken, uint256 amount, address receiver) external onlyMultiexec {
        require(cToken != C_ETH, ErrorsLib.CTokenIsCETH());
    ...
    function compoundV2RedeemEth(uint256 amount, address receiver) external onlyMultiexec {
