...
contract AaveV3MigrationAdapter is CoreAdapter {
    ...
    /// deployment).
    constructor(address initMulticall, address aaveV3Pool) CoreAdapter(initMulticall) {
    ...
    function aaveV3Withdraw(address token, uint256 amount, address receiver) external onlyInitMulticall {
        require(amount != 0, ErrorsLib.ZeroAmount());
