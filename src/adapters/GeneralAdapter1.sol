...
contract GeneralAdapter1 is CoreAdapter {
    ...
    constructor(address multiexec, address morpho, address wNative) CoreAdapter(multiexec) {
    ...
    /// underlying balance.
    function erc20WrapperDepositFor(address wrapper, address receiver, uint256 amount) external onlyMultiexec {
    ...
    /// balance.
    function erc20WrapperWithdrawTo(address wrapper, address receiver, uint256 amount) external onlyMultiexec {
        require(receiver != address(0), ErrorsLib.ZeroAddress());
    ...
    function morphoFlashLoan(address token, uint256 assets, bytes calldata data) external onlyMultiexec {
        require(assets != 0, ErrorsLib.ZeroAmount());
    ...
    function transferFrom2(address token, address receiver, uint256 amount) external onlyMultiexec {
        require(receiver != address(0), ErrorsLib.ZeroAddress());
    ...
    /// @param receiver The address that will receive the tokens.
    /// @param amount The amount of token to transfer. Pass `type(uint).max` to transfer the initiator's balance.
    function erc20TransferFrom(address token, address receiver, uint256 amount) external onlyMultiexec {
        require(receiver != address(0), ErrorsLib.ZeroAddress());
    ...
    function wrapNative(uint256 amount, address receiver) external onlyMultiexec {
    ...
    function unwrapNative(uint256 amount, address receiver) external onlyMultiexec {
