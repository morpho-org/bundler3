...
contract GenericModule1 is BaseModule {
    ...
    // using: https://git.com/morpho-org/erc20-permissioned
    ...
    /// @notice Approves the given `amount` of `token` to be spent by `spender` via ERC20 approve.
    /// @param asset The address of the ERC20 token to approve.
    /// @param spender The address allowed to spend the tokens.
    function erc20ApproveMaxToIfAllowanceZero(address asset, address spender) external bundlerOnly {
        require(asset != address(0), ErrorsLib.ZeroAddress());
        require(spender != address(0), ErrorsLib.ZeroAddress());
        ModuleLib.approveMaxToIfAllowanceZero(asset, spender);
    }
